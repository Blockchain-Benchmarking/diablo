...
func (c *DynamicWorkload) Run(accounts []blockchain.Account, _ map[string]*network.Secondary, coordinator *core.Coordinator, endpoints []string) error {
	tps := baseTps
	limit := maxTps
	...
	users, err := benchmark.CreateStubbornUsersFromAccounts(accounts, tps, "ethereum", endpoints, defaultStubbornPaymentUser)
...
bench:
	for {
		...
		updatedUsers, err := benchmark.CreateStubbornUsersFromAccounts(accounts, tps, "ethereum", endpoints, defaultStubbornPaymentUser)
	...
	file, err := os.Create("plots/dynamic/coordinates.json")
	if err != nil {
