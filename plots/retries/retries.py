...
def plot_cdf(latencies, label, total_transactions):
    ...
    cdf = np.arange(1, len(sorted_latencies) + 1) / total_transactions
    plt.plot(sorted_latencies, cdf, label=label)

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python retries.py <file1> <file2>")
        sys.exit(1)
    ...
    if latencies1:
        ...
        plot_cdf(latencies1, label=os.path.basename(file1).lstrip(".json"), total_transactions=total1)
        ...
        plt.grid(True)
        plt.tight_layout()
        output_file1 = f"{os.path.basename(file1).lstrip('.json')}_latency_cdf.png"
        plt.savefig(output_file1)
        plt.close()
        print(f"CDF plot saved to {output_file1}.")

    else:
        print(f"No successful transactions found in {file1}.")

    if latencies2:
        plt.figure(figsize=(8, 6))  # Create a new figure for file2
        plot_cdf(latencies2, label=os.path.basename(file2).lstrip(".json"), total_transactions=total2)
        ...
        plt.grid(True)
        plt.tight_layout()
        output_file2 = f"{os.path.basename(file2).lstrip('.json')}_latency_cdf.png"
        plt.savefig(output_file2)
        plt.close()
        print(f"CDF plot saved to {output_file2}.")
