...
type BurstPayment struct{}

func (c *BurstPayment) Run(accounts []blockchain.Account, secondaries map[string]*network.Secondary, coordinator *core.Coordinator, endpoints []string) error {
	defaultStubbornStoreUser, ok := benchmark.UserTypes["stubbornStoreUser"]
	if !ok {
		return fmt.Errorf("stubbornStoreUser not implemented")
	}

	wk, err := benchmark.CreateStubbornUsersFromAccounts(accounts[:300], 600, "ethereum", endpoints, defaultStubbornStoreUser)
	if err != nil {
	...
	burstPaymentUser, ok := benchmark.UserTypes["stubbornPaymentUser"]
	if !ok {
		return fmt.Errorf("stubbornPaymentUser not implemented")
	}

	burstPaymentUser.Params["duration"] = "30s"
	wk, err = benchmark.CreateStubbornUsersFromAccounts(accounts[300:], 3000, "ethereum", endpoints, burstPaymentUser)
	if err != nil {
		return fmt.Errorf("failed to create users: %w", err)
	}
